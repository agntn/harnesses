...
description: The same eleven tools over MCP, the Pi extension and the OMP extension. What they read, what they run, what they write.
---
...
| Tool | Kind | Does |
| --- | --- | --- |
| `harnesses_detect` | read | Installed harnesses and their versions |
...
| `harnesses_prompts_sync` | write | Sync canonical Markdown prompts into harness destinations, or check |
| `harnesses_skills_sync` | write | Link harness skills directories to the XDG data skills folder, or check |
...
Its text shows every path once, under `resolved`, expanded for the machine the tool runs on and filtered to its platform. The `~` templates with their platform tags are what `harnesses info` prints. An agent doesn't need both.
...
The answer comes back as plain text: a short TOON block with the id, the invocation, exit code and timeout state, then `stdout:` and the output on the lines below, capped at 8000 characters per stream, terminal control sequences stripped. A failed run adds `stderr:` the same way, and a run that printed nothing on stdout shows its stderr instead. The output is not quoted into a TOON string, so a markdown answer with code blocks reads as markdown.
