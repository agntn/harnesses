...
## Six dialects, one shape

Each harness keeps its MCP servers in its own file and its own shape. Claude, Codex, Gemini, Grok, OMP and most others use the `{ command, args, env, url }` family, in JSON or TOML. OpenCode nests them differently, Copilot reads the VS Code file, Antigravity has its own, and Prime Agent takes that family but only accepts `env` values as `{"env": "NAME"}` references. Mastra Code takes the family too but ignores `type`, so its dialect reads the transport from `command` or `url` alone. The registry records the file, the format, the object key and the dialect per harness; the library reads and writes through that:
...
```jsonc
{
  ...
  "mcpServers": {
    ...
    "wiki": { "command": "~/.local/bin/wiki-mcp", "env": { "WIKI_ROOT": "${WIKI_ROOT}" } },
...
report.targets[0]; // { id, path, results: [{ name, action: "added" | "replaced" | "removed" | "unchanged" | "skipped", reason? }] }
```
...
An `env` value that is exactly `${NAME}` is a reference to the variable `NAME` of the harness environment. Prime Agent keeps it as `{"env": "NAME"}`, because its kernel refuses literal values and resolves the reference itself when the server starts. Every other harness gets the value from the environment of the process running the sync, the same moment `${HOME}` expands. A server the target cannot hold, a literal for Prime Agent or a reference to a variable that is not set for the others, is reported as `skipped` with the reason and the rest of the list still syncs. `addMcpServer` applies the same rule and throws `McpEnvError` instead.
