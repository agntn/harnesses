...
## Five dialects, one shape

Each harness keeps its MCP servers in its own file and its own shape. Claude, Codex, Gemini, Grok, OMP and most others use the `{ command, args, env, url }` family, in JSON or TOML. OpenCode nests them differently, Copilot reads the VS Code file, Antigravity has its own, and Prime Agent takes that family but only accepts `env` values as `{"env": "NAME"}` references. The registry records the file, the format, the object key and the dialect per harness; the library reads and writes through that:
