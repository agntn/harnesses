...
- **`modelListing`** needs `parseModelListingOutput(stdout)` overridden to turn the CLI's output into `AvailableModel[]`; the base implementation throws. Leave `searchArgs` out when the CLI can't filter: a search then runs `args` and keeps the ids that contain it.
