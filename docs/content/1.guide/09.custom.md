...
- **The path groups** are what `resolve` expands and the CLI prints. Empty is fine and honest.
...
- **`invocation.streamArgs`** is for a CLI that prints nothing until the answer is done but can stream events instead. Plain runs append these flags, and `foldStreamOutput(stdout, complete)` turns the events back into the text the plain run prints; the base implementation throws. Claude's is the example.
- **`modelListing`** needs `parseModelListingOutput(stdout)` overridden to turn the CLI's output into `AvailableModel[]`; the base implementation throws. Leave `searchArgs` out when the CLI can't filter: a search then runs `args` and keeps the ids that contain it. Override `modelSelector(model)` too when the CLI's `--model` doesn't take the default `provider/id`, since `harnesses models` prints what it returns.
...
- **`temp`** and **`envOverrides`** default to empty. An override's `relocates` has to name exactly the categories with entries under its `path`, and a test holds every built-in to that. `resolve` moves `temp` when the variable is set.
