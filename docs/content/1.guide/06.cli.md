...
description: The harnesses command, ten subcommands, JSON and TOON output on all the read ones.
---
...
| `harnesses prompts sync [id] [--check]` | Sync one XDG data prompt directory across harnesses |
| `harnesses skills sync [id] [--check]` | Link every skills directory to the XDG data skills folder |
...
`--tools` and `--read-only` pick the mode, `--json` asks for the harness's structured output, `--model` a model id in the harness's own format, `--cwd` the working directory, `--timeout` a budget in seconds. `--read-only` implies `--tools`, and `--no-tools` beside it is rejected. A mode the harness cannot provide is rejected before anything runs, with the same message the library gives.
